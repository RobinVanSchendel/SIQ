    ...
    mainPanel(
      tabsetPanel(id="tabs", type = "tabs",
                  ...
                  tabPanel("About",
                           ...
                           p("You can find more information on SIQ ", a(href="https://github.com/RobinVanSchendel/SIQ", "here", target="_blank")),
...
server <- function(input, output, session) {
  ...
  tornadoPlotData <- reactive({
    ...
    if(sortType == "Start position"){
      ...
      newdata <- newdata[order(newdata$typeOrig,-newdata$size), ] 
    }
...
#max = max(in_data()[in_data()$Type=="DELETION",]$homologyLength,na.rm=T),
 ...
 # })
  ...
  typeplot <- function(el, types = c(), fraction = "relative"){
    if(fraction=="relative"){
      test2 = el %>% group_by(Subject, Alias) %>%  dplyr::count(Type = Type, wt = fraction) %>%   mutate(fraction = n / sum(n))
    }
    else{
      test2 = el %>% group_by(Subject, Alias) %>%  dplyr::count(Type = Type, wt = fraction) %>%   mutate(fraction = n ) 
    }
    #convert
