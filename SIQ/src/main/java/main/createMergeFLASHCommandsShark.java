...
public class createMergeFLASHCommandsShark {
	...
	public static void main(String[] args) throws IOException {
		...
		File dir = new File("W:\\Targeted\\GenomeScan107536\\raw");
		String linuxDir = "/exports/humgen/rvanschendel/TargetedSequencingGenomeScan107536/rawMerged/";
		
		String[] files = dir.list((dir1, name) -> name.contains("_R1"));
		Arrays.sort(files);
		for(String R1: files) {
			//safety, ensure R1 is only found once
			int index = R1.indexOf("_R1");
			int index2 = R1.indexOf("_R1", index+1);
			if(index2 != -1) {
				System.err.println("File name contains _R1 multiple times, please fix the code!!");
				System.err.println(index+"\t"+index2);
				System.err.println(R1);
				System.exit(0);
			}
			
			String R1temp = linuxDir+R1;
			String R2 = R1.replace("_R1" , "_R2");
			String R2temp = linuxDir+R2;
			String out = R1.replace("_R1.fastq.gz", "");
			String outdir = linuxDir.replace("rawMerged", "merged");
			if(outdir.contentEquals(linuxDir)) {
				System.err.println("linuxDir remains unchanged");
				System.exit(0);
			}
			System.out.println(R1temp+"\t"+R2temp+"\t"+out+"\t"+outdir);
		}
